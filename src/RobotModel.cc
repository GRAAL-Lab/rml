...
Eigen::Matrix6d RobotModel::GetIsolatedVehicleJacobianForJoint(int armIndex, int jointIndex) const throw (std::exception)
{
	if(vehicle_){
		if (CheckArm(armIndex)) {
			Eigen::Matrix6d vJv = vehicle_->GetvJv();
...
Eigen::MatrixXd RobotModel::GetArmJacobianTF(int armIndex)
{
	...
	for (int i = 0; i < arms_.size(); ++i) {
		...
		if(i != armIndex) tempJ.setZero();
		totJac = RightJuxtapose(totJac, tempJ);
	}
	return totJac;
}

Eigen::MatrixXd RobotModel::GetVehicleJacobianTF(int armIndex){
	Eigen::MatrixXd totJac, tempJ;
	if(vehicle_){
